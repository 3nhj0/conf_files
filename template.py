...
def sl(payload, after=None):
    payload = conv2bytes(payload)
    if after is None:
        io.sendline()
    else:
        io.sendlineafter(conv2bytes(after), payload)

        
'''
    output = (output.split("def start")[0] + add_func + "def start" + output.split("def start")[1])
